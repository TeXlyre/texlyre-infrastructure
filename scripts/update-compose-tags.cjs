...
Updates buildable service image tags to match the exact Git tag checked out in
the source submodule. With no file arguments, all docker-compose*.yml/yaml files
in the repository root are inspected. A leading "v" is removed from semantic
version tags (v1.2.3 -> 1.2.3), and other invalid Docker tag characters become
hyphens.`);
}
...
function exactGitTag(source) {
    try {
        return execFileSync(
            'git',
            ['-C', source, 'describe', '--tags', '--exact-match', 'HEAD'],
            { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] },
        ).trim();
    } catch {
        throw new Error(`Source "${source}" is not checked out at an exact Git tag`);
    }
}
...
function dockerTagFor(gitTag) {
    ...
    if (!tag) throw new Error(`Git tag "${gitTag}" cannot be converted to a Docker tag`);
...
function serviceUpdates(composeFile, compose) {
    ...
    for (const [name, service] of Object.entries(compose.services ?? {})) {
        ...
        if (!sourceValue) {
            throw new Error(`Service "${name}" has x-publish but no source or build context`);
        }

        const source = path.resolve(directory, sourceValue);
        const gitTag = exactGitTag(source);
...
function replaceImageLines(content, updates, composeFile) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        const imageMatch = line.match(/^(\s{4}image:\s*)(["']?)([^\s#"']+)\2(\s*(?:#.*)?)$/);
        if (!imageMatch) continue;

        const newline = raw.endsWith('\r\n') ? '\r\n' : raw.endsWith('\n') ? '\n' : '';
        lines[index] = `${imageMatch[1]}${imageMatch[2]}${update.image}${imageMatch[2]}${imageMatch[4]}${newline}`;
        applied.add(currentService);
    }

    for (const service of updates.keys()) {
        if (!applied.has(service)) {
            throw new Error(`Could not locate image line for service "${service}" in ${composeFile}`);
        }
    }
...
for (const composeFile of composeFiles) {
    ...
    for (const [service, update] of updates) {
        console.log(`${composeFile}: ${service} -> ${update.image} (${update.gitTag})`);
    }
